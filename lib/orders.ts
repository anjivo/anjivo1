...
export async function createCustomerOrder(
  ...
  if (!isValidAddress(input.shippingAddress)) {
  ...
  for (const cartItem of cart.items) {
    ...
    const productSellerId = clean(product.sellerId);
    ...
    if (productSellerId !== cartItem.sellerId) {
      throw new Error(
    ...
    if (
      cartItem.pricingType === "wholesale" &&
    ...
    const wholesaleTiers = Array.isArray(product.wholesaleTiers)
      ? product.wholesaleTiers
          ...
          .filter((tier: any) => tier.minQuantity > 0 && tier.price >= 0)
      : [];

    let selectedPrice: number;

    if (cartItem.pricingType === "wholesale") {
      selectedPrice = getWholesalePrice(
        {
          id: productSnap.id,
          name: product.name || cartItem.name,
          slug: product.slug || cartItem.slug,
          ...
          selectedPrice: Number(product.wholesalePrice ?? 0),
          pricingType: "wholesale",
        },
        cartItem.quantity
      );
    } else {
      selectedPrice = Number(product.retailPrice ?? 0);
    }
    ...
    const itemSubtotal =
      selectedPrice * cartItem.quantity;

    const sellerName =
      clean(product.sellerName) ||
      clean(cartItem.sellerName) ||
      "ANJIVO Seller";
    ...
    const orderItem: OrderItem = {
      ...
      name: clean(product.name) || cartItem.name,
      slug: clean(product.slug) || cartItem.slug,
      image:
      ...
      mrp: Number(product.mrp ?? cartItem.mrp ?? 0),
      selectedPrice,
    ...
    if (!sellerIds.includes(productSellerId)) {
      sellerIds.push(productSellerId);
    }
  ...
  const orderRef = await addDoc(collection(db, "orders"), {
    ...
    shippingAddress: {
      fullName: clean(input.shippingAddress.fullName),
      phone: clean(input.shippingAddress.phone),
      addressLine1: clean(
      ...
      city: clean(input.shippingAddress.city),
      state: clean(input.shippingAddress.state),
      pincode: clean(input.shippingAddress.pincode),
    },
